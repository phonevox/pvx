...
class ModuleUninstallScreenTest(unittest.TestCase):
    ...
    @patch("pvx.interactive.screens.module_uninstall.widgets.message")
    ...
    def test_confirmed_removal_calls_uninstall_shows_message_and_pauses(
        self, mock_discover, mock_ask_select, mock_ask_confirm, mock_uninstall, mock_message, mock_pause
    ):
        result = ModuleUninstallScreen().render()
        self.assertEqual(result, "BACK")
        mock_uninstall.assert_called_once_with("dummy")
        mock_message.assert_called_once_with("dummy removido.")
    ...
    def test_declined_confirmation_does_not_uninstall_or_pause(
        self, mock_discover, mock_ask_select, mock_ask_confirm, mock_uninstall, mock_pause
    ):
        result = ModuleUninstallScreen().render()
        self.assertEqual(result, "BACK")
